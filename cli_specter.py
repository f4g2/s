...
from gdt.utils import get_scidocs_metadata, write_func_args
...
def extract_triples(
    ...
    from specter_dataset import DataReaderFromPickled
...
def find_train_ids(
    ...
    python cli_specter.py find_train_ids ${SPECTER_DIR}/train_triples.csv \
        ...
        --s2orc_paper_ids ./data/s2orc/entity_names_paper_id_0.json \
    ...
    # S2-S2ORC Mappings
    with open(s2id_to_s2orc_input_path) as f:
    ...
    logger.info(f'S2ORC paper IDs: {len(s2orc_paper_ids):,}')
    ...
    logger.info(f'Unique S2 IDs in Scidocs: {len(scidocs_s2_ids):,}')
    ...
    logger.info(f'Loaded {len(specter_train_triples):,} SPECTER triples from {specter_triples_path}')
    ...
    logger.info(f'SPECTER papers with S2ORC ID and in graph: '
